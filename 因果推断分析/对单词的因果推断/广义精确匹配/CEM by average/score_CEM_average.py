...
output_data_path = os.path.join(current_dir, '匹配文件/score_CEM_average_results.xlsx')
output_results_path = os.path.join(current_dir, '匹配文件/score_CEM_average_summary.xlsx')
...
def mean_match_fill(scores, indices, column):
    matched_means = []
    for index_list in indices:
        matched_means.append(scores.iloc[index_list][column].mean())
    return matched_means

# 对处理组D=1的S0进行填充
treated_scores['matched_S0'] = mean_match_fill(control_scores, indices, 'S0')
...
matched_S1_means = mean_match_fill(treated_scores, indices, 'S1')
