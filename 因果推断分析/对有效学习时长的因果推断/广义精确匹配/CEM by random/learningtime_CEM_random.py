...
output_file_path_filled = os.path.join(current_dir, '匹配文件/learningtime_CEM_random_results.xlsx')
output_results_path = os.path.join(current_dir, '匹配文件/learningtime_CEM_random_summary.xlsx')
...
treated_group = data_for_psm[data_for_psm['D'] == 1]
control_group = data_for_psm[data_for_psm['D'] == 0]
...
# 从匹配的对照组样本中随机选择T0进行填充
matched_control_t0 = []
for idx_list in indices_treated:
    selected_t0 = control_group.iloc[idx_list]['T_combined'].sample(n=1).values[0]
    matched_control_t0.append(selected_t0)
...
# 从匹配的干预组样本中随机选择T1进行填充
matched_treated_t1 = []
for idx_list in indices_control:
    selected_t1 = treated_group.iloc[idx_list]['T_combined'].sample(n=1).values[0]
    matched_treated_t1.append(selected_t1)
