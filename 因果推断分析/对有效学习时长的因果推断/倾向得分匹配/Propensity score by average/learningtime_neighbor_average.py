...
output_file_path_filled = os.path.join(current_dir, '匹配文件/learningtime_neighbor_average_results.xlsx')
output_file_path_results = os.path.join(current_dir, '匹配文件/learningtime_neighbor_average_summary.xlsx')
...
matched_control_t0 = [control_group.iloc[idx_list]['T_combined'].mean() for idx_list in indices_treated]
...
matched_treated_t1 = [treated_group.iloc[idx_list]['T_combined'].mean() for idx_list in indices_control]
